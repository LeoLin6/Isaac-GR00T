...
python eval_gr00t_kbot.py \
    ...
    --host=10.112.209.136 \
    --port=5555 \
    --lang_instruction="Pick up the bottle and place it in the container." \
    --cam_left_idx=0 \
    --cam_right_idx=1 \
...
# Add the path to your zbot_inspire module
sys.path.append(os.path.join(os.path.dirname(__file__), "zbot_inspire"))
...
class Gr00tKBotInferenceClient:
    ...
    def get_action(self, img_left, img_right, state):
        obs_dict = {
            "video.left": img_left[np.newaxis, :, :, :],
            "video.right": img_right[np.newaxis, :, :, :],
            "state.left_arm": state[:5][np.newaxis, :].astype(np.float64),
            "state.right_arm": state[5:10][np.newaxis, :].astype(np.float64),
            "state.right_hand": state[10:16][np.newaxis, :].astype(np.float64),
    ...
    def sample_action(self):
        obs_dict = {
            ...
            "state.left_arm": np.zeros((1, 5)),
            "state.right_arm": np.zeros((1, 5)),
            "state.right_hand": np.zeros((1, 6)),
            "annotation.human.task_description": [self.language_instruction],
        }
